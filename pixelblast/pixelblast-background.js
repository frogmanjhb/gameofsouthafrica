...
import { EffectComposer, EffectPass, RenderPass, Effect } from 'https://cdn.jsdelivr.net/npm/postprocessing@6.33.3/+esm';
...
export class PixelBlastBackground {
  constructor(container, options = {}) {
    if (!container) {
      throw new Error('PixelBlastBackground requires a container element');
    }
  ...
  init() {
    ...
    const setSize = () => {
      ...
      if (this.composer) this.composer.setSize(renderer.domElement.width, renderer.domElement.height);
    };
    ...
    this.onPointerDown = e => {
      ...
      const ix = this.clickIndex % MAX_CLICKS;
    ...
    this.onPointerMove = e => {
      if (!isInside(e) || !this.touch) return;
  ...
  animate() {
    ...
    if (this.options.autoPauseOffscreen && !this.visible) {
      this.raf = requestAnimationFrame(this.animate);
      return;
    }

    const { renderer, scene, camera, composer, touch, liquidEffect, uniforms } = this;
    uniforms.uTime.value = this.timeOffset + this.clock.getElapsedTime() * this.options.speed;

    if (liquidEffect) {
      const uniformTime = liquidEffect.uniforms.get('uTime');
      if (uniformTime) uniformTime.value = uniforms.uTime.value;
    }
